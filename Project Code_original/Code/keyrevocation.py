...
def keyrevocation(target_section):
    ...
    if target_section == "0":   #target section = admin section
        ...
        admincol = mydb["admin"]
        admincol.delete_many({})
        
        # re-encryptf patient's document recover
        directory_admin = 'admin'.format(target_section)
        for filename in os.listdir(directory_admin): #encrypt local patient's documents and upload to MongoDB
            f = os.path.join(directory_admin, filename)
            ...
            if os.path.isfile(f):
                ...
                with open('admin.key'.format(target_section),'rb') as file2:
                    new_key = file2.read()
                doc_encrypted = symcrytjson.encryptjson(new_key,data_string,"") 
                admincol = mydb["admin"]
                admincol.insert_one(doc_encrypted)
        print("Finished re-encryption of admin's documents")
